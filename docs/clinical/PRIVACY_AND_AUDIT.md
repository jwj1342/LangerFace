...
- 本地优先、位置透明：标准 RSTL 和浏览器算法不上传画面；用户显式启动 V10 时，工作帧只发送到页面标示的局域网电脑或受控远程 V10 服务。
- 最小化：只采集完成验证或审阅所必需的坐标、版本和诊断信息。
- 可追溯：任何图谱、候选切口或医生覆盖都必须能追到作者、时间、版本和依据。
- 可撤回：受限数据应有明确所有者、保存期限和删除路径。
- 医生确认：系统不得把候选线描述为自动手术指令。

Vercel 生产入口必须启用 Deployment Protection 或等效的终端用户访问控制。Vercel Function 只签发
90 秒、来源绑定、单次使用的检测令牌，不接收图像正文；远程 V10 服务按签名主体限流，只允许配置的
网页来源，45 秒超时或浏览器取消时终止任务，并在响应结束后删除临时目录。诊断日志不得记录令牌或像素。
...
- 离线重建结果默认只写入 gitignored 的 `local_outputs/`；不得复制到网页公开资产或提交真实患者头模。
