...
- [x] RSTL 局部方向服务：为切口设计提供方向、置信度与可解释依据 — [#13](https://github.com/jwj1342/LangerFace/issues/13)
      · 本 PR 已支持 atlas weighted-nearest 查询、仅统计有效非零切向的 support count、点序无关的规范无向轴、JS/Python 对拍和结构化 `confidence_reasons`；空 atlas 与非空但无有效方向支持均返回完整 JSON-safe 低置信契约。共享 fixture 覆盖空 atlas、单点、重复/零长度、缺字段、未知方向 metadata 和点序反转，并用同一 held-pose 人脸连续 100 帧的隐私最小化检测派生输入固定轴向角范围 ≤ 6°、相邻帧变化 ≤ 1.5°、标准差 ≤ 2°、最低置信度 ≥ 0.95；前端会区分 RSTL atlas、敏感结构方向例外、辅助线索只读和医生人工覆盖；临床级 atlas 质量与个体头模一致性仍待医生校验
...
- 肌肉骨骼实时孪生、术中级软组织/肌肉骨骼耦合模拟：当前不属于 Stage 2 切口 Agent 目标。未来如重启，需另开决策 gate，详见 [PRODUCT_BOUNDARIES.md](../clinical/PRODUCT_BOUNDARIES.md)。
