...
- [ ] 真实媒体验收：照片、视频与摄像头切口叠加稳定性证据 — [#172](https://github.com/jwj1342/LangerFace/issues/172)
      ...
      · PR #235 时间受限合并后须补做视野受限候选专项：在同一原始输入、审阅记录、最终合并提交和运行身份下，以单 Chromium、`workers=1` 串行覆盖正脸、左右转头、回正、候选部分出视野后恢复及极端偏航门禁；核对隐藏片段重新显露的位置与连续性、抖动诊断，以及照片端确认路径与实时 `polyline_refs` 的逐点/表面引用同一性。当前只有表面引用映射和照片可见片段契约的低资源测试通过，不能据此宣称真实摄像头已验证或会按照片确认形状自动补全。失败时不得放宽红色硬阻断，应先统一“照片确认完整几何 -> 审阅记录 -> overlay payload -> 当前帧可见裁剪”的数据契约再复测。
...
- [x] 隐私、合规与审计记录：限定临床研究使用边界 — [#21](https://github.com/jwj1342/LangerFace/issues/21)
      · 本 PR 已更新 README/隐私文档、导出字段边界、`privacy_audit` 字段、浏览器 `browser-export-privacy-preflight/v0.1` 和 `tools/audit_export_privacy.py`，可在导出或分享审阅/肿物/诊断 JSON 前拦截原始媒体标记、未脱敏 secret、明显身份字段、电话邮箱模式、疑似嵌入媒体 payload，以及辅助线索越界参与几何的标记；共享 review gate 保留保守默认策略，合并工作流不强制备注，但仍会阻止未确认、缺 reviewer、存在不可覆盖工程错误或 trace gate 未过的候选标成实时叠加就绪；真实临床访问控制、日志保留、签名和 DPA 仍需合规流程
