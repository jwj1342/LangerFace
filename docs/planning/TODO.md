...
- [x] 实时 3D 重建不恢复用户入口；生产维持 2D-first，3D 仅作离线资产、图谱标注和研究预览 — [#40](https://github.com/jwj1342/LangerFace/issues/40)
      · 量化 gate、三条路线边界与重启条件见 [3D 路线 ADR](../architecture/ADR_3D_ROUTE_FEASIBILITY.md)；#4、#26、#27 随该决策关闭为 not planned，不删除 `/annotate` 或 `/incision` 仍在使用的 3D 可视化基础设施。
