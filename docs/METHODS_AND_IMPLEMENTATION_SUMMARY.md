...
- 与局部 RSTL 的角度偏差。
- 是否命中敏感结构 guardrail。
- `candidate_version`、`parent_candidate_id`、`edit_id` 和 `edit_history`；工具生成候选为 v1，医生参数编辑或端点拖拽后的候选至少为 v2，多步编辑会按提交顺序递增版本并写入 `candidate-edit-session/v0.1`，导出报告显示候选版本和编辑记录数。
- `tumor_boundary_summary`；审阅记录会按保存候选的长轴重新汇总肿物边界点数、长短轴、面积、自交和中心偏移，Markdown 报告同时显示“肿物边界摘要”和“梭形包络”，`tools/audit_incision_review_gate.py` 会重放该摘要并核对梭形候选 metrics，避免 reviewer 只看到候选线而漏看输入边界质量。
