...
- 审阅最小工作流：支持保存多个候选、生成方向备选、记录审阅人/确认/退回/否决状态和备注，并导出 JSON、Markdown 报告草案和 PNG 截图；导出记录包含 `guardrail_summary` 和 `review_gate`；候选几何调整后审阅状态会回到待医生确认；高风险 guardrail 候选确认前必须填写审阅备注或覆盖原因，发送到实时叠加前必须先确认候选草案。
...
- LLM Agentic：LLM 只做摘要和解释，工具 schema、trace 和 stream 契约已固化，前端会实时展示工具 trace；长程自主规划仍必须受确定性工具、审计记录和医生确认约束。
