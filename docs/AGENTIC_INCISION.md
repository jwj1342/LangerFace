...
- Agent 编排：LLM 只读取工具结果并生成摘要，不计算几何、不覆盖安全规则；Python agent、SSE 代理和前端会用 `agent-trace-gate/v0.1` 检查 trace 至少包含肿物输入质量、面部分区、RSTL 查询、确定性切口生成和 guardrails，未通过时不能确认候选。后端现在会在主 RSTL 方向旁生成 `-10° / 0° / +10°` 三个确定性方向候选，逐个复跑切口生成与 guardrails，再用 `compare_candidates` 输出工程排序；`agent-orchestration-audit/v0.1` 会记录候选数、比较状态、恢复失败数和临床边界。确定性候选变体生成失败时会追加 `recover_tool_failure` trace，跳过失败变体，并继续比较剩余候选；`tests/test_incision_agent.py` 用 synthetic variant failure 覆盖这条恢复路径。
...
- 验证汇总：`tools/evaluate_stage2_validation.py` 可读取脱敏审阅 JSON，汇总候选类型、医生确认率、guardrail 分布、RSTL 偏角、梭形几何误差、敏感距离、`incision-overlay-registration/v0.1` runtime projection QA、`incision-overlay-stability/v0.1` 抖动 RMS/P95/max 与通过率、失败模式和隐私审计计数。
...
- LLM Agentic：LLM 只做摘要和解释，工具 schema、trace、stream 契约、工具门控、后端方向备选、候选比较工具、失败恢复审计和离线 trace audit 已固化，前端会实时展示工具 trace、Agent 工具门控和后端候选比较；`agent-trace-gate/v0.1` 由 Python agent 输出并通过 SSE `trace_gate` 事件暴露，会把缺失工具动作、顺序异常和确定性几何缺失写入审阅记录，并阻断未过门控的确认候选。当前已测试覆盖确定性候选变体失败后的 `recover_tool_failure` 记录、候选跳过和剩余候选继续比较；`tools/audit_agentic_incision_trace.py` 也可离线重放 trace gate 与候选比较，检查导出记录是否可复现。但它仍是单轮多候选工具链，不是完整长程自主规划系统，后续多轮计划和跨工具自动重试执行器仍必须受确定性工具、审计记录和医生确认约束。
