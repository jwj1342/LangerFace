...
- Agent 编排：LLM 只读取工具结果并生成摘要，不计算几何、不覆盖安全规则；Python agent、SSE 代理和前端会用 `agent-trace-gate/v0.1` 检查 trace 至少包含肿物输入质量、面部分区、RSTL 查询、`inspect_sensitive_structures` 敏感结构检查、确定性切口生成、guardrails 和 `preview_incision_on_face` 面部预览，未通过时不能确认候选。`inspect_sensitive_structures` 会先在候选生成前记录中心点/保护性方向例外，再在每个成功候选生成后复核该候选几何到敏感游离缘的最短距离；审阅导出保存的是主候选生成后的复核观察，每个后端方向备选也会携带自己的 `sensitive_structure_inspection`。后端现在会在主 RSTL 方向旁生成 `-10° / 0° / +10°` 三个确定性方向候选，逐个复跑切口生成、敏感结构复核、guardrails 与面部预览，再用 `compare_candidates` 输出工程排序；`agent-react-plan/v0.1` 会把肿物检查、解剖定位、RSTL 查询、敏感结构检查、主候选生成、guardrail 检查、候选预览和方向变体比较记录成可复放步骤；`agent-execution-events/v0.1` 会把同一条 trace 派生成 UI/SSE 可消费的执行事件序列，包含 tool observation、trace gate 评估、ReAct plan 评估、重试和恢复状态；`agent-orchestration-audit/v0.1` 会记录候选数、preview 数量、可渲染 preview 数量、比较状态、ReAct 计划状态、重试次数、恢复失败数和临床边界。确定性候选变体生成失败时会先追加 `retry_tool_failure` trace 并对同一工具有界重试一次；重试仍失败才追加 `recover_tool_failure` trace、跳过失败变体，并继续比较剩余候选；`tests/test_incision_agent.py` 用 transient / persistent synthetic variant failure 覆盖这两条路径。
...
- 审阅最小工作流：支持保存多个候选、生成方向备选、工程排序比较、记录审阅人/确认/退回/否决状态和备注，并导出 JSON、Markdown 报告草案和 PNG 截图；导出记录包含 `tumor_boundary_summary`、`sensitive_structure_inspection`、`guardrail_summary`、`review_gate`、`agent_trace_gate`、`agent_react_plan`、`agent_execution_events` 和 `candidate_comparison`；Markdown 报告会显示肿物边界摘要、中心点/候选几何敏感结构检查和梭形 outline/envelope 指标；候选几何调整后审阅状态会回到待医生确认；高风险 guardrail 候选确认前必须填写审阅备注或覆盖原因，发送到实时叠加前必须先确认候选草案，且 Agent 工具门控必须通过。浏览器 JSON 导出前会运行 `browser-export-privacy-preflight/v0.1`，阻断 raw media 标记、明文 secret、直接身份字段/电话邮箱模式、嵌入式媒体 payload 或辅助线索越界参与几何/Agent prompt；`tools/audit_incision_review_gate.py` 可离线审计审阅导出中的 `review_gate` 与 `live_overlay_ready` 是否和审阅人、高风险备注、审阅状态、`agent_trace_gate` 自洽，并对皮表边界审阅记录重放 `tumor_boundary_summary`、核对 `sensitive_structure_inspection` 是否匹配 Agent trace observation、核对梭形候选边界 metrics。
...
- Agent 工具 schema：`assets/agentic_incision_tool_schema.json` 固化工具名、输入输出、`agent-react-plan/v0.1`、`agent-execution-events/v0.1`、`agentic-incision-session/v0.1`、`inspect_sensitive_structures`、`preview_incision_on_face`、`propose_direction_variants`、`compare_candidates`、`retry_tool_failure`、`recover_tool_failure` 和 LLM 边界。
...
- 前端 Provider 配置：工作台主配置只暴露 LLM Provider mode、Base URL、model、API Key、timeout 和“测试 LLM Provider 连接”；Ollama Native 测试 `/api/tags`，OpenAI-compatible 测试 `/models`。Vercel 预览不会默认填 `127.0.0.1` / `localhost`；如果用户手动填写，会先提示该地址指向浏览器所在机器并继续发送测试请求，方便在 DevTools 里看到真实 CORS/Private Network 结果。规划后端 endpoint 收在“高级：规划后端接口”里，默认同域 `/api/agentic-incision` 且默认不启用，API Key 导出记录中会脱敏。
...
- 医生审阅：当前支持候选保存、方向备选、工程排序比较、审阅人、审阅状态、备注、确认/否决动作、JSON/Markdown/PNG 导出和 `audit_events`；工具生成候选从 `candidate_version=1` 开始，医生参数编辑和端点拖拽会生成 `parent_candidate_id`、`edit_id`、`candidate_version` 和多步 `edit_history`，前端提供撤销/重做与编辑时间线状态；“生成备选”会优先保存后端 `candidate_alternatives`，保留每个方向备选自己的 guardrails、preview、敏感结构复核和候选比较上下文，没有后端备选时才使用浏览器本地编辑 fallback。审阅 JSON 额外写入 `candidate-edit-session/v0.1`、`tumor_quality`、`tumor_boundary_summary` 和 `sensitive_structure_inspection`，报告会显示候选版本、编辑记录数、肿物输入提示、肿物边界摘要、中心点/候选几何敏感结构检查与梭形包络指标；`tools/audit_incision_review_gate.py` 会核对 `guardrail_summary` 是否忠实反映原始 `guardrails.warnings` 和 `suggested_overrides`，也会核对 `candidate_edit_session` 与候选 provenance 的版本、编辑步数、当前 edit_id 和每步 resulting version，并核对 `sensitive_structure_inspection` 是否来自同一条 Agent trace，防止审阅导出里的风险摘要、敏感结构观察和编辑时间线误导 reviewer，并检查线性候选端点/中心/长度和梭形 outline/polyline/长短轴字段是否自洽；编辑后的候选会重新计算覆盖、包络和 guardrail 状态，候选比较只按 guardrails、RSTL 偏角、覆盖缺口、敏感距离和几何误差做工程排序，不是临床推荐。正式电子签名、病例系统绑定、权限控制和审阅锁定属于后续受控临床系统集成。
...
打开 `incision_agent.html`。页面默认不启用规划后端，先使用浏览器内确定性工具，避免静态预览自动请求不存在的 `/api/agentic-incision`。如果只是测试 Ollama/OpenAI-compatible 是否通，直接在 LLM Provider 区填写 Base URL / Model / API Key 后点击“测试 LLM Provider 连接”。Vercel 预览里应填写可从浏览器访问、且允许该 preview origin 的 HTTPS Provider 地址；`127.0.0.1:11434` 指的是打开浏览器那台机器，不是 Vercel 或远端服务。需要后端 trace/LLM 摘要时，再在“高级：规划后端接口”里启用后端 Agent 编排。
