...
## 7. 界面或入口合并必须验证“行为能力”，不能只核对组件和正向路径

**现象**：把两个已有工作台合并到一个页面后，主要按钮、表单和算法调用都在，页面也能生成正常结果，
但部分功能仍静默退化：鼠标扫描范围没有显示、原页面已有的非法区域前置门禁没有接入，甚至底层已经返回
“禁止显示”，新绘制层仍继续画出结果。

**根因**：合并实现把原工作台当成一组可见组件来迁移，并重新编写了一层适配控制器；验收主要覆盖
“按钮存在、点击能生成”的正向路径，没有逐项迁移原状态机中的视觉反馈、失败路径、负向门禁、状态清理和
绘制抑制条件。算法没有改变，但算法外围的行为契约被适配层漏接，最终表现得像算法失效。

**通用规则（不限前端、语言或业务领域）**：

1. 合并前先做能力矩阵，至少逐项列出：输入、触发事件、可见反馈、成功输出、失败反馈、负向门禁、状态清理、导出和生命周期；每项标记为“复用 / 有意移除 / 需要迁移”，不能只列按钮。
2. 适配层优先调用原有共享策略和门禁函数，不复制条件判断；确需重新编排时，为正向路径和每个硬阻断条件各写一条反向测试。
3. 同一结果跨多层传递时，测试最终消费者是否遵守状态字段；“算法返回 blocked”不等于“界面没有显示”，必须验证绘制、计数、导出和交接都停止。
4. 新旧入口并存期间，用同一组输入做差分验收；不仅比较结果数据，也比较交互反馈、错误提示、隐藏/禁用状态和退出后的恢复状态。
5. 在能力矩阵和差分验收完成前，合并页只能标为开发入口，不能因主流程可运行就声明功能等价。

**为什么“已经加了守卫”仍可能无效**：

1. 守卫放错数据边界。坐标吸附、归一化、序列化、聚合等转换可能丢失原始语义；在转换后检查，函数虽然执行了，却已看不到应拦截的事实。硬门禁应尽量放在第一次有损转换之前，并在最终消费者再做一次兜底。
2. 测试只证明代码结构，不证明运行行为。正则检查函数名存在、调用顺序看似正确，只能防止明显漏接；它不能证明传入的是正确坐标、完整范围或当前状态。关键门禁必须用可执行输入断言返回值和副作用，并至少包含一个会在删除/挪后该门禁时失败的反例。
3. 状态失效没有所有权边界。一个模块参数变化时，若直接重启共享帧、重置共享 store 或重发完整快照，其他模块会被无关刷新。迁移时应区分“本模块草案重算”“共享显示重绘”“底层媒体重新分析”，只有真正依赖变化的层级可以被触发。
4. 错误优先级没有契约。多个检查可能同时失败时，后执行的通用错误会掩盖更具体、更可操作的前置错误。需要明确错误优先级，并用同一输入同时触发两个失败条件来验证最终文案。

**迁移完成后的自查顺序**：

1. 用旧入口和新入口跑同一组最小样本，逐项比较输入、可见反馈、结果、错误、清理和恢复，而不是只比较最终成功结果。
2. 画出事件与状态所有权：谁能写共享媒体、谁能请求新帧、谁只能更新自己的 overlay；逐个操作确认不会触发无关模块。
3. 标出每种数据的坐标域、单位和生命周期，特别检查有损转换前后的门禁、缓存失效和异步过期结果。
...
- [ ] 合并/迁移工作台时按能力矩阵核对正向、失败、门禁、反馈、清理、导出和生命周期；底层 `blocked` 必须在最终消费者处验证。
