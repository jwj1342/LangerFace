...
- **Phase 1（触发：多用户在线标注持久化 / 在线服务受限头模）**：起 Worker + D1 + R2，实现 `ApiDataSource`；当前已新增 `workers/api` 第一版瘦 API、D1 migration、合约测试和 CI 验证，正式 Cloudflare 资源绑定与前端 `ApiDataSource` 切换仍待后续完成。落地步骤、GitHub Actions 接入、Cloudflare token 权限和验收清单见 [CLOUDFLARE_BACKEND_ROLLOUT.md](CLOUDFLARE_BACKEND_ROLLOUT.md)。
