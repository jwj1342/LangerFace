...
Python 实现在 `src/langerface/lines/mapping.py`，Web 实现在 `web/src/services/geometryAtlas.ts` 的 `mapAtlas`。
...
转头时，背侧脸部线条不应该显示。系统基于三角面法向做自遮挡剔除，实现位于 `src/langerface/rendering/occlusion.py` 和 `web/src/services/geometryAtlas.ts`。
...
### 7.1 React 实时页的额头皮肤/头发可见性

`web/src/services/foreheadVisibility.ts` 对额头点使用图像证据，不等价于上面的三角面法向。像素先从
...
最后沿曲线填补长度不超过 `1.5%` 的小缺口、删除短于 `2.5%` 的可见段；整条曲线的可见点少于
`22%` 时整体隐藏，否则保留所有达到最短长度的可见段。它由 React live 的 TypeScript 渲染路径
与法向、pose gate、局部区域质量及遮挡组合使用。
...
Python 端用 OpenCV 抗锯齿折线绘制，位于 `src/langerface/rendering/overlay.py`。Web 端用 Canvas 2D 绘制，位于 `web/src/services/render2d.ts`。
...
Web 端入口由 React SPA、`web/src/services/liveRuntime.ts`、`web/src/services/pipeline.ts`、`web/src/services/render2d.ts`、`web/src/services/liveState.ts` 等模块组成，支持摄像头、上传图片和上传视频。
...
Web 端 `web/src/services/three3d.ts` 会计算顶点法向，并将线点沿法向微抬，减少 z-fighting：
...
仓库还包含 FLAME 3D 轨道，位于 `src/langerface/flame.py`、`web/src/services/flameFit.ts` 和相关工具脚本。
...
`/app/annotate` React 路由、`web/src/components/Annotate*.tsx`、`web/src/services/annotateRuntime.ts`、`web/src/services/annotationModel.ts`、`web/src/services/annotateViewer.ts` 共同实现网页 3D 线标注工具；`web/annotate.html` 仅保留兼容跳转。
...
`web/src/services/rstlField.ts` 从图谱线条恢复每个线点的 3D 位置和切向：
...
`web/src/services/softBody.ts` 把标准脸表面边建成弹簧网络。
