...
当前产品范围采用方案 B：主开发与阶段联合验收只覆盖单个、轮廓清楚、整体近似圆形（允许轻度椭圆）的皮表肿物，明显狭长椭圆不自动纳入。下文自由轮廓、包络和异常门禁仍是底层架构兼容能力，保留是为了不破坏既有输入、审计和回归契约，不表示当前产品承诺处理任意复杂不规则肿物。
...
共享审阅策略默认要求高风险 guardrail 候选记录备注，并默认把受限参考保留为待确认草案。合并工作流根据当前医生操作约定显式采用较简化的审阅策略：不强制填写备注；普通、视野受限和非标准比例参考在医生主动确认、工作流 trace 完整且不存在不可覆盖的工程几何错误时可以进入实时叠加。红色虚线诊断候选始终禁止确认和叠加。实时入口仍只接受已确认候选，避免把未审阅草案误当成可用方案。
...
`/personalized` 暂存的 YOLO/V6 个体化 RSTL；缺失、来源不符或拓扑校验失败时，切换到内置
