...
### 3.2 采集质量门控（`QUALITY_THRESHOLDS`）

`tracking ≥ 0.62`、`illumination ≥ 0.55`、`returnConsistency ≥ 0.58`、`minPeakFrames ≥ 4`。

单轮采集靠**质量门控**保证可用性，而不是靠重复多轮取平均——重复轮次会显著拉长采集时间，
却主要在补偿"用户没做到动作"，而这一点用门控判断更直接。`returnConsistency` 检查动作后能否回到静息，
用来排除头部移动被误当成表情形变的情况。
