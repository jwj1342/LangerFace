...
- [ ] Cloudflare Worker/D1/R2 后端基础设施：先落地本地验证、少量云端资源创建、GitHub Actions 部署和 `ApiDataSource` 接口
      · 本 PR 已新增 `workers/api` 第一版 Worker：包含病例、肿物、标注、候选切口、医生审阅和脱敏导出的 D1 schema/API、CORS、明显身份字段拦截、高风险审阅备注约束、fake D1 合约测试和 CI `worker-tests`；Worker 只做瘦 API、鉴权、CORS、D1/R2 访问和状态流转，不承载 MediaPipe/FLAME/OpenCV/Blender 等重计算。仍待创建/绑定正式 Cloudflare D1/R2 资源、配置 GitHub Secrets / Vercel `VITE_API_BASE_URL`，并在前端新增可切换的 `ApiDataSource`。落地流程见 [CLOUDFLARE_BACKEND_ROLLOUT.md](CLOUDFLARE_BACKEND_ROLLOUT.md)。
