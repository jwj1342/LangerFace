...
- [ ] 临床验证数据集与评估指标：从演示原型走向可验证研究系统 — [#20](https://github.com/jwj1342/LangerFace/issues/20)
      · 本 PR 已补齐 Stage 1/2 指标、验证集格式、失败分类和 `tools/evaluate_stage2_validation.py`，可把脱敏 `incision-review-record/export` 汇总为候选类型、医生确认率、guardrail 分布、RSTL 偏角、梭形几何误差、敏感距离、`incision-overlay-registration/v0.1` runtime projection QA、`incision-overlay-stability/v0.1` 抖动 RMS/P95/max、`rstl-local-region-quality-gate/v0.1` 局部降可信 active region/action/source/reason、`landmark-motion-stabilized-smoothing/v0.1` 平滑启用率/method/参数分布、`incision-overlay-3d-view-diagnostics/v0.1` 3D 预览 rendered/failure、`incision-overlay-replay-qa/v0.1` 离线复放 QA 与通过率、`overlay_local_region_quality_review` / `overlay_3d_view_failure` 等失败模式和隐私审计计数，并可用 `--csv-output` 导出 reviewer 可读的扁平汇总表；真实病例库和医生统计验证仍需受控临床流程
