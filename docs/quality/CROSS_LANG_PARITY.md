...
本文定义线条几何在 **Python 与 Web TypeScript** 两套实现间的**逐点等价不变式**、活体对拍机制，
...
2. **vis** —— 背面剔除 **且** 排除 #38 口裂（内唇）三角面后的逐三角面可见性。两端**逐位精确**相等。
...
- **生产数学合法变更后**（改了映射、背面剔除、口裂集合、One-Euro 常量等）：
  ...
  随后跑全部门禁（`ruff` / `pytest` / `npm run build` / `npm test`）确认两端仍一致。
