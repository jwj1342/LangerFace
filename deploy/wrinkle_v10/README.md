...
1. The browser calls the same-origin `/api/wrinkle-v10` endpoint.
2. The Vercel function in `web/api/wrinkle-v10.mjs` forwards the binary request.
3. This container runs the repository's `run_live_four_region_wrinkle.py --serve`
...
2. Keep the generated `WRINKLE_V10_SERVICE_TOKEN` secret.
3. Copy the deployed service URL and token into the Vercel project environment:

   - `WRINKLE_V10_SERVICE_URL=https://<render-service-host>`
   - `WRINKLE_V10_SERVICE_TOKEN=<same generated token>`

4. Redeploy the Vercel web project with `web` as its root directory.
5. Verify `GET https://<web-host>/api/wrinkle-v10` returns:
...
HTTP 429 for concurrent requests. Requests are cancelled after 45 seconds, and
temporary pixel/request files are removed after each response.
