...
export function draw(lm, W, H, masks = []) {
  ...
  const stride = refineActive ? 1 : Math.max(1, Math.round(100 / (renderState.densityFrac * 100)));
  ...
  if (refineActive && selectedLine != null && selectedPoint != null) {
    const line = displayLines[selectedLine];
    const point = line?.pts?.[selectedPoint];
    if (point) {
      ctx.globalAlpha = 1;
