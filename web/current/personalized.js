 ...
 * 算法：采集 ./prstl_pipeline.js；个性化 ./bottom_up_personalization.js
 * 定位：非个体 RSTL 测量
 *
 * 分区：
 *   1. 常量与 DOM
...
import personalizedAtlasUrl from "../assets/atlas_rstl_personalized.json?url";
...
const SECOND_CYCLE_HINT = {
  raise_brows: "先完全放松眉毛，再以与第 1 轮相近的幅度缓慢抬眉",
  frown: "先松开眉间，再重复两眉向中间靠拢，避免低头代偿",
  squint: "先睁眼放松，再双眼轻眯；务必保留眼裂，不要用力闭眼",
  smile: "先恢复自然嘴角，再重复相近幅度的自然微笑",
  puff: "先排出空气放松脸颊，再均匀鼓起两侧脸颊",
  purse: "先放松嘴唇，再以相近幅度撅嘴，避免张嘴",
  open_mouth: "先自然闭口，再重复张嘴并保持头部不后仰",
};

// 降低阈值，便于普通摄像头采集（THRESHOLDS 来自 pipeline）
const HOLD_NEED = 0.75; // 秒
const NEUTRAL_NEED = 15;
const CYCLES_REQUIRED = 2;
...
function renderSteps() {
  ...
  els.steps.innerHTML = rows.map((r, i) => {
    ...
    const cycleCount = r.id === "neutral" ? 0 : (sess?.actionCycles?.[r.id]?.length || 0);
    const round = r.id === cur && cycleCount === 1 ? " · 1/2 已完成" : "";
    return `<div class="${cls}"><span class="n">${i + 1}</span><span>${r.label}${round}</span></div>`;
  }).join("");
}
function syncStartButton() {
  ...
  if (sess.stage === "actions" && (sess.phase === "paused" || sess.phase === "countdown")) {
    ...
    els.start.disabled = false;
    els.start.textContent = sess.retryCurrent
      ? `重新采集：${label}`
      : sess.repeatCurrent ? `第二轮：${label}` : `下一步：${label}`;
    if (els.confirm) els.confirm.disabled = true;
    return;
  }
  els.start.disabled = true;
...
function updateGuide() {
  ...
  if (sess.stage === "neutral") {
    ...
    if (phase === "paused") {
      const secondRound = (sess.actionCycles?.[a]?.length || 0) === 1;
      const verb = sess.retryCurrent ? "重新采集" : secondRound ? "第 2/2 轮" : "第 1/2 轮";
      els.guideTitle.textContent = `${label} · ${verb}`;
      els.guideSub.textContent = secondRound
        ? `第 1 轮已通过。${SECOND_CYCLE_HINT[a]}`
        : "点击后倒计时；完成动作并保持到进度结束，即提交本轮";
      els.coach.classList.remove("hidden");
      els.coachTitle.textContent = label;
      els.coachSub.textContent = secondRound ? SECOND_CYCLE_HINT[a] : (ACTION_HINT[a] || "点击下方按钮开始");
      setMeters(0, 0);
      ...
      const cycle = Math.min(CYCLES_REQUIRED, (sess.actionCycles?.[a]?.length || 0) + 1);
      els.guideTitle.textContent = `${label} · 第 ${cycle}/${CYCLES_REQUIRED} 轮`;
    ...
    els.guideSub.textContent = "粉线=最终中性模板（全部 132 条），灰线=标准先验，黄线=有证据位移；左侧列出偏移最大的线";
    els.coach.classList.add("hidden");
    setMeters(0, 1);
  }
  syncStartButton();
...
function drawLegend(cctx, x, y) {
  ...
  cctx.fillText("灰线：初始 atlas（全部 132 条）", x + 20, y + 18);
  cctx.strokeStyle = "rgba(250,204,21,0.8)";
  cctx.beginPath(); cctx.moveTo(x, y + 33); cctx.lineTo(x + 14, y + 33); cctx.stroke();
  cctx.fillStyle = "#fde68a";
  cctx.fillText("黄线：发生观测位移处", x + 20, y + 36);
}
 ...
 */
function renderCompareImage(imageData, seeds, curves, size = SIZE) {
  ...
  // 1) 初始 atlas：全部 132 条完整灰线
  for (const cc of list) {
    const prior = cc.priorPts || cc.pts;
    strokeFull(cctx, prior, 1.2, "rgba(150,160,175,0.7)", null);
  }

  // 2) 位移短线（黄）：逐点 prior→final，仅画实际发生位移处
  if (curves && curves.length) {
    cctx.strokeStyle = "rgba(250,204,21,0.6)";
    cctx.lineWidth = 0.8;
    cctx.setLineDash([]);
    for (const cc of curves) {
      ...
      for (let i = 0; i < m; i += stepN) {
        ...
        if (!a || !b) continue;
        if (Math.hypot(a[0] - b[0], a[1] - b[1]) < 0.6) continue;
        cctx.beginPath();
        cctx.moveTo(a[0], a[1]);
        cctx.lineTo(b[0], b[1]);
        cctx.stroke();
...
/**
 * Bottom-up optimizer boundary. The guided capture state machine owns all
 * inputs and remains independent of the patient-texture curve warper.
 */
function updateBottomUpNeutralTemplate(trigger) {
  if (!sess) return [];
  const texture = sess.bottomUpTexture || sess.staticEvidence;
  const cameraQuality = sess.sourceFaceMetrics?.quality ?? 0.72;
  const result = warpPriorCurvesWithHessian({
    seeds: sess.displaySeeds || sess.seeds || [],
    textureQ: texture?.q || sess.q0,
    textureConfidence: texture?.confidence || new Float32Array(SIZE * SIZE),
    textureRidge: texture?.ridge || new Float32Array(SIZE * SIZE),
    dynamicValidation: sess.dynamicValidation,
    skin: sess.skin,
    forbidden: sess.forbidden,
    size: SIZE,
    options: {
      // Resolution changes confidence, not eligibility. Low-resolution cameras
      // therefore fall back more often but are never blocked by a fixed width.
      minimumEvidence: 0.055 + 0.045 * (1 - cameraQuality),
      minimumDirectionAgreement: 0.58 + 0.08 * (1 - cameraQuality),
      smoothingPasses: 14,
    },
  });
  sess.curves = result.curves;
  ...
  sess.optimizationDiagnostics = {
    ...result.diagnostics,
    source_face_quality: cameraQuality,
    trigger,
    updated_at: new Date().toISOString(),
  };
  recordDebugEvent(result.diagnostics.fallback ? "bottom_up_optimizer_fallback" : "bottom_up_optimizer_applied", {
    trigger,
    diagnostics: sess.optimizationDiagnostics,
  });
  return sess.curves;
}
...
function renderDebugMediaExports() {
  ...
  els.debugMediaStatus.textContent = active
    ? `正在记录：${active.actionLabel} · 第 ${active.round} 轮（已保存 ${clips.length} 段）`
    : clips.length
  ...
  els.debugMediaExports.innerHTML = clips.map((clip) => {
    if (!clip.blob) return `<span class="hint">${clip.actionLabel} 第 ${clip.round} 轮：仅数据（无视频）</span>`;
    if (!clip.downloadUrl) {
      clip.downloadUrl = URL.createObjectURL(clip.blob);
      retainedMediaUrls.push(clip.downloadUrl);
    }
    return `<a href="${clip.downloadUrl}" download="${clip.fileName}">下载 ${clip.actionLabel} · 第 ${clip.round} 轮视频（${(clip.bytes / 1024 / 1024).toFixed(1)} MB）</a>`;
  }).join("");
}
...
function appendCaptureSample({ bs, alignedMesh, pose, poseQuality, sourceFaceWidth, detectedFace = true }) {
  const active = sess?.activeRecording;
  if (!active) return;
...
function finishExports() {
  ...
  const { canvas: cmpCanvas, cmp } = renderCompareImage(neutralImage, seeds, curves, SIZE);
  ...
  if (els.compareSummary) {
    ...
    els.compareSummary.innerHTML =
      `<div style="font-size:13px;font-weight:700;margin-bottom:2px">最终曲线 ${curves.length}/${seeds.length} · 直接证据 ${refinedLines} 条 · 连续场移动 ${movedLines} 条</div>
       <div class="hint" style="margin-bottom:6px">粉=最终中性模板 · 灰=标准先验 · 黄=位移；无证据区域由连续场衰减或保持先验</div>
  ...
  // 并排：左初始 atlas（灰，132） 右最终个性化（粉，132）
  try {
...
async function aggregateCycleEvidence(frames, action) {
  ...
  for (const item of best) {
    ...
    const flow = blockMatchFlow(sess.neutralGray, fr.grayLow, SIZE, SIZE, sess.skin, 12, 2);
...
function commitCycle(action) {
  ...
  setTimeout(async () => {
    ...
    try {
      ...
      const gate = evaluateQualityGate(cycle.quality, { ...QUALITY_THRESHOLDS, returnConsistency: 0 });
      ...
      if (cycles.length < CYCLES_REQUIRED) {
        debugRecord.outcome = "accepted_waiting_for_round_2";
        recordDebugEvent("cycle_accepted", { action, round: 1, outcome: debugRecord.outcome });
        sess.phase = "paused";
        sess.retryCurrent = false;
        sess.repeatCurrent = true;
        sess.cycleFrames = [];
        sess.returnFrames = [];
        sess.message = `「${ACTION_LABELS[action]}」第 1/2 轮有效。${SECOND_CYCLE_HINT[action]}`;
        processing = false;
        updateGuide();
        setMsg(sess.message);
      ...
      const regionWeight = actionRegionField(action, sess.regionMasks, SIZE);
      const repeatability = repeatabilityScore(cycles[0], cycles[1], regionWeight);
      const repeatDecision = decideRepeatability(action, repeatability, sess.directionRetries[action] || 0);
      ...
      if (!repeatDecision.accept) {
        debugRecord.outcome = "repeatability_rejected";
        cycles.pop();
        sess.directionRetries[action] = (sess.directionRetries[action] || 0) + 1;
        sess.rejectedCycles.push({ action, reasons: ["repeatability"], repeatability });
        const tip = action === "squint"
          ? "眯眼方向证据不足：请双眼轻眯并保留眼裂，不要闭眼、低头或挤鼻子，然后重做第 2 轮"
          : "两轮方向重复性不足，请按相同幅度重做第 2 轮";
        throw new Error(tip);
      }
      const current = mergeRepeatedCycles(cycles[0], cycles[1]);
      ...
      updateBottomUpNeutralTemplate(`action:${action}`);
      const optimizerFallback = !!sess.optimizationDiagnostics?.fallback;
      const acceptedTargets = sess.optimizationDiagnostics?.accepted_targets || 0;
      debugRecord.outcome = optimizerFallback
        ? "accepted_but_bottom_up_fell_back_to_prior"
        : acceptedTargets > 0
          ? "accepted_and_bottom_up_warp_applied"
          : "accepted_but_evidence_below_bottom_up_commit_threshold";
      ...
      // 每步融合后刷新对比预览
      try {
        const { canvas: prev, cmp } = renderCompareImage(sess.neutralCanonical || sess.lastCanon, sess.displaySeeds || sess.seeds, sess.curves, SIZE);
        if (els.compareCanvas) {
        ...
        if (els.compareSummary) {
          const rc = sess.curves.filter((c) => (c.refinedFrac || 0) > 0).length;
          const mc = sess.curves.filter((c) => (c.movedFrac || 0) > 0).length;
          els.compareSummary.innerHTML =
            `<div style="font-size:12px">已融合 ${(sess.completed?.length || 0) + 1} 步 · 直接证据 ${rc} 条 · 局部证据场移动 ${mc} 条</div>
             <div class="hint">粉=最终中性模板 · 灰=初始先验 · 黄=位移</div>`;
        }
      } catch (_) {}
      ...
      if (!next) {
        ...
        sess.stage = "done";
        sess.phase = "done";
        sess.message = `完成！共 ${sess.curves.length} 条微调曲线`;
        finishExports();
        looping = false;
        setLive(false);
      } else {
        sess.actionIndex = ACTION_ORDER.indexOf(next);
        // 已前进到下一步，等用户点「下一步：xxx」
        sess.phase = "paused";
        const resultNote = optimizerFallback
          ? `Bottom-Up 微调未找到可提交目标，已安全保留先验：${sess.optimizationDiagnostics?.fallback_reason || "未知原因"}`
          : acceptedTargets > 0
            ? `方向验证通过，Hessian 接受 ${acceptedTargets} 个局部目标`
            : "采集有效，但证据低于 Bottom-Up 提交门槛，已保留先验";
  ...
  if (sess.stage === "neutral") {
    ...
    if (sess.neutralGrayFramesHi.length >= NEUTRAL_NEED) {
      try {
        ...
        const prior = rasterizePrior(atlasLines, mesh3, triangles, SIZE);
        sess.q0 = prior.q0;
        sess.seeds = prior.seeds;
        // 全部 atlas 线都参与精修；可读性靠逐点分类着色（只有 refined 才粉色）
        sess.displaySeeds = prior.seeds;
...
async function beginCurrentAction() {
  ...
  const roundHint = roundNumber === 2 ? SECOND_CYCLE_HINT[next] : ACTION_HINT[next];
  ...
  try {
    for (const n of [3, 2, 1]) {
      ...
      showCountdown(n, n === 3 ? `${label} · 第 ${roundNumber}/2 轮` : n === 2 ? roundHint : "马上开始！");
      sess.message = `${label} · 第 ${roundNumber}/2 轮 · ${n}`;
      setMsg(sess.message);
      await sleep(900);
    }
    showCountdown("开始", `第 ${roundNumber}/2 轮：请做「${label}」并保持`);
    await sleep(450);
  } finally {
    hideCountdown();
  }
...
async function skipStep() {
  ...
  if (!next) {
    ...
    if (sess.fieldQ && sess.q0) {
      updateBottomUpNeutralTemplate(`skip_after:${action}`);
    } else if (seedsForTrace) {
      sess.curves = seedsForTrace.map((s) => ({
        name: s.name, pts: s.pts, priorPts: s.pts, kinds: s.pts.map(() => "prior"),
