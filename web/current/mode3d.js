...
export function setMode3d(m) {
  ...
  reconState.mode3d = m;
  els.view3d.textContent = reconState.reconDisplaySpace === "model" ? "查看 FLAME 头模" : "旋转查看";
  els.view3d.setAttribute("aria-pressed", String(m === "view"));
  els.project3d.setAttribute("aria-pressed", String(m === "project"));
  els.project3d.textContent = !reconState.reconProjectable
    ? "扫描后可投影"
    : (m === "project" ? "返回 3D 模型" : "投影到画面");
...
export async function loadDemoRecon() {
  ...
  reconState.reconAtlasLines = modelState.flameRstlOverlay?.atlas?.lines || [];
  reconState.reconProjectVerts = null;
...
function finishScan(collected, ymin, ymax, colorFrames = [], lowDepthConfidence = false, durationMs = null) {
  ...
  const scanVerts = verts.map((p) => [p[0] - c[0], p[1] - c[1], p[2] - c[2]]);
  reconState.reconProjectVerts = scanVerts;
  reconState.reconColors = null;
  try {
    const basis = reconState.flameBasis || modelState.flameRstlOverlay?.basis;
    if (!basis) throw new Error("FLAME basis unavailable");
    const beta = fitShape(scanVerts, basis).beta;
    const psi = fitExpression(scanVerts, basis, beta).psi;
    reconState.flameBeta = beta;
    reconState.reconVerts = flameForward(basis, beta, psi, 0);
    reconState.reconFaces = facesArray(basis);
    reconState.reconAtlasLines = modelState.flameRstlOverlay?.atlas?.lines || [];
    reconState.reconDisplaySpace = "model";
  } catch (err) {
    logWarn("扫描结果拟合 FLAME 失败，回退到 468 扫描壳。", err);
    reconState.reconVerts = scanVerts;
  ...
  els.reconStatus.textContent = lowDepthConfidence
    ? `扫描完成并已拟合到 FLAME（深度置信度低：偏航跨度 ${(ymax - ymin).toFixed(2)} < ${YAW_SPAN_MIN}，投影需复核）：${N} 帧。可旋转查看 FLAME / 投影。`
    : `扫描完成并已拟合到 FLAME：${N} 帧，偏航 ${ymin.toFixed(2)}~${ymax.toFixed(2)}。可旋转查看 FLAME / 投影。`;
  buildViewer();
}
