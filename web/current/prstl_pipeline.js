...
const THRESHOLDS = {
  raise_brows: 0.18, frown: 0.15, squint: 0.18, smile: 0.22,
  puff: 0.06, purse: 0.08, open_mouth: 0.18,
};
// 这些动作 blendshape 不可靠：点击开始后按计时完成（仍采集帧做融合）
export const TIMED_ACTIONS = new Set(["puff", "purse"]);
...
export function landmarksToCanonicalXY(lmPx, size, refXY = null) {
...
// ── 先验栅格化 ──────────────────────────────────────────────────────────────
export function rasterizePrior(atlasLines, refXY, triangles, size) {
  const mapped = mapAtlas(atlasLines, refXY.map((p) => [p[0], p[1], 0]), triangles);
...
export function pickBestFrames(neutralGray, frames, skin, k = 3) {
  ...
  for (let idx = 0; idx < frames.length; idx++) {
    ...
    scored.push({ fr, idx, score: n ? acc / n : 0, gray: g });
  }
