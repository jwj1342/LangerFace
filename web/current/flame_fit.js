...
export function sampleFlameLandmarks(verts, basis, targetLandmarkCount = Infinity) {
  const flame = [], targetIndices = [], faceIndices = [];
  for (let i = 0; i < basis.NL; i++) {
    const targetIndex = basis.landmarkIndices[i];
    if (targetIndex >= targetLandmarkCount) continue;
    const faceIndex = basis.lmkFaceIdx[i];
    const a = basis.faces[faceIndex * 3];
    const b = basis.faces[faceIndex * 3 + 1];
    const c = basis.faces[faceIndex * 3 + 2];
    const w0 = basis.lmkBCoords[i * 3];
    const w1 = basis.lmkBCoords[i * 3 + 1];
    const w2 = basis.lmkBCoords[i * 3 + 2];
    flame.push([0, 1, 2].map((axis) =>
      w0 * verts[a][axis] + w1 * verts[b][axis] + w2 * verts[c][axis]));
    targetIndices.push(targetIndex);
    faceIndices.push(faceIndex);
  }
  return { flame, targetIndices, faceIndices };
}

export function projectFlameToLandmarks(verts, landmarks, basis) {
  const sampled = sampleFlameLandmarks(verts, basis, landmarks.length);
  if (sampled.flame.length < 4) throw new Error("Not enough FLAME landmarks for camera projection");
  const target = sampled.targetIndices.map((index) => landmarks[index]);
  return {
    vertices: applySim(umeyama(sampled.flame, target), verts),
    referenceFaces: sampled.faceIndices,
  };
}
