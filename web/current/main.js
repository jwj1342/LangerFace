...
function applyStagedAtlas() {
  ...
  const isPersonalizedV6 = String(atlas.provenance || "").includes("local-yolo")
    || Boolean(atlas.diagnostics?.algorithm);
  previewSystem = atlas.system;
  previewMeta = {
    source: isPersonalizedV6 ? "个性化 V6" : "标注会话",
    validated: atlas.validated === true,
    count: atlas.lines.length,
  };
  els.tmpl.value = atlas.system;
  syncPreviewControls();
  if (!sourceState.running) setMsg(isPersonalizedV6
...
function refreshStaticImage() {
  ...
  else if (sourceState.paused) redrawPausedFrame();
}

let imageDrag = null;

function startImageDrag(e) {
  const staticEditable = sourceState.sourceKind === "image"
    || (["camera", "video"].includes(sourceState.sourceKind) && sourceState.paused);
  if (!staticEditable || e.button !== 0) return;
  if (beginRefinePointer(e)) {
    e.preventDefault();
    return;
  }
  if (sourceState.sourceKind !== "image") return;
...
els.pause.onclick = () => {
  const result = toggleSourcePause();
  if (result === "paused" && !isRefineActive()) toggleRefine2d();
};
...
els.refine2d.onclick = toggleRefine2d;
els.refineView.onclick = () => setRefineMode("view");
...
els.refineReset.onclick = resetRefineToAuto;
...
window.addEventListener("langerface:refine2d-redraw", refreshStaticImage);
