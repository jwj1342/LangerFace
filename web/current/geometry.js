...
function extendForeheadLowerLongArc(points, landmarksPx) {
  ...
  for (let i = 0; i < out.length; i++) {
    const parallel = (out[i][0] - anchor[0]) * unitX + (out[i][1] - anchor[1]) * unitY;
    out[i][0] += 0.86 * parallel * unitX;
    out[i][1] += 0.86 * parallel * unitY;
  }
  ...
  if (currentHalfWidth > 1e-9 && faceWidth > 1e-9) {
    ...
    for (let i = 0; i < out.length; i++) {
      out[i][0] += (factor - 1.0) * lateralCoordinates[i] * lateralX;
      out[i][1] += (factor - 1.0) * lateralCoordinates[i] * lateralY;
    }
  }
  ...
  const smoothedLaterals = out.map((point) => (
    (point[0] - anchor[0]) * lateralX + (point[1] - anchor[1]) * lateralY
  ));
  const halfWidth = Math.max(...smoothedLaterals.map((value) => Math.abs(value)));
  ...
  if (halfWidth > 1e-9 && faceHeight > 1e-9) {
    for (let i = 0; i < out.length; i++) {
      const normalized = Math.min(1.0, Math.abs(smoothedLaterals[i]) / halfWidth);
      const arch = 0.055 * faceHeight * (1.0 - normalized ** 2.4);
      out[i][0] += arch * unitX;
      out[i][1] += arch * unitY;
    }
  }
...
export function mapAtlas(lines, landmarksPx, triangles) {
  ...
  for (const ln of lines) {
    ...
    const mappedPoints = ln.region === FOREHEAD_LOWER_LONG_ARC_REGION
      ? extendForeheadLowerLongArc(pts, landmarksPx)
      : pts;
    result.push({
      name: ln.name,
      region: ln.region || "",
      symmetryRole: ln.symmetryRole || "",
      symmetryPairId: ln.symmetryPairId || "",
      pts: mappedPoints,
      tris,
    });
  }
...
// MediaPipe 内唇边界的有序闭环。与 INNER_LIP 三角面排除配合，防止相邻图谱点
// 都在唇外时，连接线仍横跨张开的口腔。
export const INNER_MOUTH_CONTOUR = [
  78, 191, 80, 81, 82, 13, 312, 311, 310, 415,
  308, 324, 318, 402, 317, 14, 87, 178, 88, 95,
];

export function mouthOcclusionPolygon(landmarksPx) {
  if (!Array.isArray(landmarksPx)) return [];
  return INNER_MOUTH_CONTOUR.map((index) => landmarksPx[index]).filter(Boolean);
}
...
// Ray-casting test for an arbitrary ordered polygon (clockwise or counter-clockwise).
export function pointInPolygon(p, polygon) {
  if (!p || !Array.isArray(polygon) || polygon.length < 3) return false;
  let inside = false;
  for (let i = 0, j = polygon.length - 1; i < polygon.length; j = i++) {
    const a = polygon[i], b = polygon[j];
    if (!a || !b) continue;
    const crosses = (a[1] > p[1]) !== (b[1] > p[1]);
    if (crosses && p[0] < (b[0] - a[0]) * (p[1] - a[1]) / (b[1] - a[1]) + a[0]) {
      inside = !inside;
    }
  }
  return inside;
...
// 与 visibleRuns 相同，但把跨越排除多边形的边拆开；用于张嘴时的口腔空洞。
export function visibleRunsOutsidePolygon(pts, visMask, polygon) {
  if (!Array.isArray(polygon) || polygon.length < 3) return visibleRuns(pts, visMask);
  const runs = [];
  let cur = [];
  const flush = () => {
    if (cur.length >= 2) runs.push(cur);
  ...
  for (let i = 0; i < pts.length; i++) {
    const p = pts[i];
    if (!visMask[i] || !isFinite(p[0]) || !isFinite(p[1]) || pointInPolygon(p, polygon)) {
      flush();
      continue;
    }
    if (cur.length && segmentCrossesPolygon(cur[cur.length - 1], p, polygon)) flush();
    cur.push(p);
  }
  flush();
  return runs;
