...
function collectElements(root = document) {
  return {
    canvas: $(root, "surgeryCanvas"),
    wrap: root.querySelector?.(".main-wrap") || document.querySelector(".main-wrap"),
    hint: $(root, "hint"),
    lesionState: $(root, "lesionState"),
    size: $(root, "sizeRange"),
    sizeVal: $(root, "sizeVal"),
    along: $(root, "btnAlong"),
    reset: $(root, "btnReset"),
    showLines: $(root, "showLines"),
    tensionVal: $(root, "tensionVal"),
    tensionBar: $(root, "tensionBar"),
...
function createRuntimeState() {
  return {
    ...
    mounted: false, resizeObserver: null, frameId: 0, abortController: null,
  };
}
...
function bindSurgeryEvents() {
  ...
  els.canvas.addEventListener("pointermove", (e) => {
    if (!drag || e.pointerId !== drag.id || !S.head) return;
  ...
  els.canvas.addEventListener("wheel", (e) => {
    e.preventDefault();
    S.head?.zoom(e.deltaY > 0 ? 1.1 : 0.9);
  }, { passive: false, signal });

  els.along.addEventListener("click", () => doExcision(S.dir[S.lesion], "along"), { signal });                                  // 长轴沿 RSTL
  els.reset.addEventListener("click", reset, { signal });
  els.showLines.addEventListener("change", () => { S.linesDirty = true; refreshLines(); }, { signal });
  els.size.addEventListener("input", () => {
    els.sizeVal.textContent = `${els.size.value}%`;
    if (!S.sb) updatePreview();
  }, { signal });
...
export function disposeSurgeryClosureDemo() {
  S.mounted = false;
...
export function mountSurgeryClosureDemo(root = document) {
  disposeSurgeryClosureDemo();
  ...
  boot(S).catch((err) => {
    if (!S.mounted) return;
    els.hint.textContent = "加载失败：" + err.message;
    console.error(err);
  });
