...
function formatModelVersion(snapshot: ReturnType<typeof useIncisionStore.getState>["snapshot"]) {
  const mode = snapshot?.headAsset.mode;
  if (mode === "mediapipe_personalized") return "个体化 MediaPipe 468";
  if (mode === "mediapipe_standard") return "MediaPipe 468 标准先验";
  return "待识别";
}
...
export function IncisionStatePanel() {
  ...
  return (
    <Card className="incision-state-panel">
      ...
      {snapshot?.headAsset.mode === "mediapipe_standard" ? (
        <Hint><a href="/personalized">先完成 YOLO/V6 个性化 RSTL，再进入切口设计</a></Hint>
      ) : null}
