    ...
    <>
      <div className="card">
        <div>
          ...
          <Button variant="workbench" className={atlasPreview?.active ? "" : "hidden"} id="restoreAtlasBtn" type="button" onClick={() => dispatchRenderCommand("restore_atlas")}>恢复官方图谱</Button>
        </div>
        <div>
      ...
      <div className="card">
        ...
        <label className="check">
          <Checkbox id="mirror" checked={mirror} onChange={(event) => dispatchRenderCommand("mirror_toggle", event.currentTarget.checked)} /> 镜像（自拍视角）
        </label>
        ...
        <label className="check">
          <Checkbox id="meshPts" checked={meshPts} onChange={(event) => dispatchRenderCommand("mesh_points_toggle", event.currentTarget.checked)} /> 显示网格采样点
        </label>
      </div>
    </>
  );
