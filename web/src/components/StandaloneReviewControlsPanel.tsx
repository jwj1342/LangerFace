...
export function StandaloneReviewControlsPanel() {
  ...
  useEffect(() => {
    if (snapshot?.review.status) setStatus(snapshot.review.status);
  }, [snapshot?.review.status]);

  useEffect(() => {
    setReviewer(snapshot?.review.reviewer || "");
    setNotesPresent(Boolean(snapshot?.review.notesPresent));
  }, [snapshot?.review.notesPresent, snapshot?.review.reviewer]);
  ...
  }, [decisionAttentionRequired, notesAttentionRequired, reviewSaveNotice, reviewerAttentionRequired, snapshot?.updatedAt]);

  return (
    <WorkbenchCard>
      <CardHeader>
        <span>医生审阅</span>
        <ReviewStatus tone={reviewTone(status)} id="reviewState">{reviewStatusLabel(status)}</ReviewStatus>
      </CardHeader>
      <div>
