...
export type AssetNameKey = keyof typeof assetNames;
...
export type AssetUrls = Record<string, string> & Record<AssetNameKey, string>;
...
function configuredAssetBaseUrl(): string {
  ...
  if (query) {
    ...
    try {
      localStorage.setItem(ASSET_BASE_STORAGE_KEY, normalized);
    } catch {
...
export function assetUrl(key: string): string {
  const name = assetNames[key as AssetNameKey] || key;
  return new URL(name, assetBaseUrl()).href;
}

export const assetUrls = new Proxy({} as AssetUrls, {
  get(_target, key) {
...
async function readTextWithProgress(
  resp: Response,
  onProgress?: (event: Pick<AssetProgressEvent, "loaded" | "total" | "ratio">) => void,
): Promise<string> {
...
async function readBufferWithProgress(
  resp: Response,
  onProgress?: (event: Pick<AssetProgressEvent, "loaded" | "total" | "ratio">) => void,
): Promise<ArrayBuffer> {
  const total = Number(resp.headers.get("content-length") || 0) || null;
...
async function fetchAsset<T>(
  key: string,
  { kind, label = key, onProgress = undefined }: AssetLoadOptions & { kind: "json" | "arrayBuffer" },
): Promise<T> {
...
export function loadJsonAsset<T = unknown>(key: string, options: AssetLoadOptions = {}): Promise<T> {
  return fetchAsset<T>(key, { ...options, kind: "json" });
}

export function loadArrayBufferAsset(key: string, options: AssetLoadOptions = {}): Promise<ArrayBuffer> {
  return fetchAsset<ArrayBuffer>(key, { ...options, kind: "arrayBuffer" });
}
