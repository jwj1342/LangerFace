...
export interface CreateSkinMaterialOptions {
  showSurface?: boolean;
  vertexColors?: boolean;
  fallbackColor?: THREE.ColorRepresentation;
}

export function meshBounds(verts: Vec3[]): MeshBounds {
  ...
  const center = min.map((value, axis) => (value + max[axis]) * 0.5) as Vec3;
  const extent = min.map((value, axis) => Math.max((max[axis] - value) * 0.5, 1e-5)) as Vec3;
  return { min, max, center, extent };
}

export function configureSkinRenderer(renderer: THREE.WebGLRenderer): void {
  renderer.outputColorSpace = THREE.SRGBColorSpace;
  renderer.toneMapping = THREE.ACESFilmicToneMapping;
  renderer.toneMappingExposure = 1.08;
}

export function addSkinLighting(scene: THREE.Object3D): THREE.Group {
  ...
  material.onBeforeCompile = (shader) => {
    ...
    for (const anchor of VERTEX_ANCHORS) {
      if (!shader.vertexShader.includes(anchor)) {
        logWarn("皮肤材质：顶点着色器缺少注入锚点，皮肤效果可能失效（three 版本变化？）。", { anchor });
      }
    }
    for (const anchor of FRAGMENT_ANCHORS) {
      if (!shader.fragmentShader.includes(anchor)) {
        logWarn("皮肤材质：片元着色器缺少注入锚点，皮肤效果可能失效（three 版本变化？）。", { anchor });
      }
    }
