...
- 🧭 **切口 Agent 工作台**：在标准脸上手动放置皮下 / 皮表肿物，支持椭圆 / 自由轮廓和肿物 JSON 导入导出，生成线性或梭形候选切口，显示 RSTL 方向、面部分区、guardrails、流式工具调用 trace 和 provider 状态；候选可记录审阅人、确认 / 退回 / 否决状态和备注，导出审阅记录，也可发送到实时页叠加到上传照片、视频或摄像头画面。
...
| [rstl_mediapipe_direction_prior.json](assets/rstl_mediapipe_direction_prior.json) | MediaPipe 标准脸高密度 RSTL 方向场草案（issue #86，`validated:false`） |
| **质量 / 运维** | |
