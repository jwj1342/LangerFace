...
assert.ok(js.includes("workflow_trace_gate"), "review records include deterministic workflow trace gate state");
assert.ok(js.includes("sensitive_structure_inspection"), "review records include sensitive structure inspection");
assert.ok(js.includes("workflow_plan_audit"), "review records include 工作流计划 plan state");
assert.ok(js.includes("workflow_execution_events"), "review records include deterministic workflow execution events");
...
assert.ok(js.includes("workflow_audit"), "review export includes browser workflow orchestration audit");
...
assert.ok(js.includes("工作流恢复详情"), "markdown report includes recovered failure details");
...
assert.ok(js.includes("工作流执行事件"), "markdown report includes workflow execution event status");
