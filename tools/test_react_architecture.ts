...
const architectureDoc = readRoot("docs/architecture/ARCHITECTURE.md");
const ciCdVercelDoc = readRoot("docs/quality/CI_CD_VERCEL.md");
const contributingDoc = readRoot("docs/onboarding/CONTRIBUTING.md");
...
assert.ok(!app.includes('to="/annotate" replace'), "atlas settings is no longer a direct redirect to the annotation route");
...
for (const [name, source, workspace, runtime] of [
  ["AnnotateRoute.tsx", annotateRoute, "annotate", "annotateRuntime"],
  ["IncisionRoute.tsx", incisionRoute, "incision", "incisionRuntime"],
  ["LiveRoute.tsx", liveRoute, "live", "liveRuntime"],
]) {
  assert.ok(source.includes("ManagedWorkbenchRoute"), `${name} should render through the shared managed workbench route primitive`);
  assert.ok(source.includes(`typeof import("../services/${runtime}")`), `${name} should type its TypeScript runtime module`);
...
assert.ok(typedStore.includes('"settings"'), "app store includes the settings workspace for settings route lifecycle");
assert.ok(typedStore.includes("export type WorkerStatus"), "app store exports a typed worker status union");
assert.ok(typedStore.includes("setWorkerStatus: (workerStatus: WorkerStatus)"), "app store keeps worker status updates typed");
...
assert.ok(incisionEditHistoryService.includes("class IncisionEditHistory"), "incision edit history service owns timeline state");
assert.ok(incisionEditHistoryService.includes("undo()"), "incision edit history service owns undo transitions");
assert.ok(incisionEditHistoryService.includes("redo()"), "incision edit history service owns redo transitions");
...
assert.ok(workflowWorkerProbeHook.includes('setWorkerStatus("正在连接")'), "React workflow worker probe hook publishes connecting status");
assert.ok(workerPanel.includes("useWorkflowWorkerProbe"), "React worker status panel consumes the workflow worker probe hook");
assert.ok(!workerPanel.includes("createWorkflowWorkerClient"), "React worker status panel does not create worker clients directly");
assert.ok(!workerPanel.includes("probeWorkflowWorkerClient"), "React worker status panel does not probe Comlink directly");
...
assert.ok(annotateRuntime.includes("../lib/controllerEvents"), "annotation runtime imports event names from the shared module");
...
assert.ok(annotateRuntime.includes("handleReactLineLibraryCommand"), "annotation runtime routes React saved line commands to existing workflow functions");
assert.ok(annotateRuntime.includes("./annotateSnapshots"), "annotation runtime consumes the shared typed snapshot service");
...
assert.ok(!annotateRuntime.includes("CustomEvent(ANNOTATE_CONTROLLER_STATE_EVENT"), "annotation runtime does not hand-roll state snapshot CustomEvent dispatch");
...
assert.ok(liveCommandRouterService.includes("readLiveRouteCommand(event)"), "live route handler validates command names and route values");
...
assert.ok(liveController.includes("liveCommands.handleRouteEvent(event)"), "live controller routes React route commands through the shared router");
...
assert.ok(liveController.includes('liveCommands.route("project_3d")'), "compatibility route controls reuse the shared command router");
...
assert.ok(surgeryRoute.includes("useStandardFaceAssets"), "React surgery route lazy-loads closure demo assets through the shared standard face hook");
assert.ok(!surgeryRoute.includes("loadStandardFaceAssets"), "React surgery route should not duplicate standard face loader wiring");
