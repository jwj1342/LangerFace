...
{
  const source = readFileSync(new URL("../web/current/personalized.js", import.meta.url), "utf8");
  ok(!source.includes("enterReturnPhase("), "采集流程不再进入回到自然表情状态机");
  ok((source.match(/commitCycle\(action\);/g) || []).length >= 3, "自动与手动动作完成均直接提交本轮");
  ok(source.includes("cycles.length < CYCLES_REQUIRED"), "回退后仍保留两轮采集流程");
  ok(source.includes("decideRepeatability(action, repeatability"), "回退后仍保留两轮方向一致性验证");
}
