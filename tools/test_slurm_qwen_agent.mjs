...
const providerModeEnv = ["LANGERFACE", "LLM", "PROVIDER"].join("_");
...
assert.ok(!script.includes(`${providerModeEnv}=`), "agent provider mode is fixed in code and not configurable");
...
const providerIndex = script.indexOf("export LANGERFACE_LLM_BASE_URL=\"http://127.0.0.1:$VLLM_PORT/v1\"");
...
assert.ok(providerIndex > healthIndex, "provider endpoint env is exported after vLLM health check");
