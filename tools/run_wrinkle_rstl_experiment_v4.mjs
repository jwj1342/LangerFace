...
const fineLinePath = resolve(
  projectRoot,
  "langer线-cc/wrinkle_extraction_experiment_v1/wrinkle_fine_lines.json",
);
...
try {
  ...
  if (!status?.startsWith("实验完成")) {
    throw new Error(`${status}\n${browserMessages.join("\n")}`);
  }
  const data = await page.evaluate(() => window.__wrinkleRstlExperiment.artifactData());
