...
def test_v8_1_69_adds_symmetric_supraorbital_short_arc_bundles(tmp_path):
    ...
    assert v69_payload["validated"] is False
    assert v69_payload["atlasVersion"] == "8.1.69"
    assert len(v69_payload["lines"]) == 159
    assert sum(len(line["points"]) for line in v69_payload["lines"]) == 15272
    assert v69_payload["lines"][-1]["name"] == "standard_field_0174_left"
    ...
    assert sum(line["region"] == LATERAL_REGION for line in v69_payload["lines"]) == 8
    assert sum(line["region"] == MEDIAL_REGION for line in v69_payload["lines"]) == 10
