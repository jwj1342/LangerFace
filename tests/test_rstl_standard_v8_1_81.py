...
def test_v8_1_81_follows_lateral_canthus_red_guide_only(tmp_path):
    ...
    assert v81_payload["atlasVersion"] == "8.1.81"
    assert v81_payload["validated"] is False
    assert len(v81_payload["lines"]) == EXPECTED_LINE_COUNT
    assert sum(len(line["points"]) for line in v81_payload["lines"]) == EXPECTED_POINT_COUNT

    old_lines = {line["name"]: line for line in v80_payload["lines"]}
    new_lines = {line["name"]: line for line in v81_payload["lines"]}
    ...
    for name in changed_names:
        ...
        assert old_lines[name]["postMapSmoothingPasses"] == 12
        assert new_lines[name]["postMapSmoothingPasses"] == 24

    old_atlas = Atlas.load(str(tmp_path / "atlas_v80.json"))
    new_atlas = Atlas.load(str(tmp_path / "atlas_v81.json"))
    ...
    for source_index in TARGET_SOURCE_INDICES:
        ...
        assert old_ratio < 0.80
        assert new_ratio > 1.20
        assert new_ratio > old_ratio + 0.40
        assert _terminal_slope(right) >= 4.0 * _terminal_slope(
