...
def test_v8_1_90_deepens_and_lengthens_orbital_brow_spoons(tmp_path):
    ...
    assert v90_payload["atlasVersion"] == "8.1.90"
    assert v90_payload["validated"] is False
    assert len(v90_payload["lines"]) == 190
    assert sum(len(line["points"]) for line in v90_payload["lines"]) == 18_258

    old_lines = {line["name"]: line for line in v89_payload["lines"]}
    new_lines = {line["name"]: line for line in v90_payload["lines"]}
    ...
    for name in TARGET_NAMES:
        ...
        assert np.isclose(new_spec[2][1] - old_spec[2][1], 0.008, atol=1e-12)
    ...
    old_atlas = Atlas.load(str(tmp_path / "atlas_v89.json"))
    new_atlas = Atlas.load(str(tmp_path / "atlas_v90.json"))
    ...
    for frame in frames:
        ...
        for side in ("right", "left"):
            ...
            for name in names:
                ...
                assert new_depth - old_depth > 0.0015 * face_width
        ...
        assert max(frame_extensions) > 0.01 * face_width
