...
def test_v8_1_91_further_deepens_and_lengthens_orbital_brow_spoons(tmp_path):
    canonical = CanonicalFaceModel.from_obj(CANONICAL_OBJ)
    v90_payload = _atlas_payload(
        canonical,
        ROOT / "assets/rstl_standard_reference_v8_1_90.json",
        tmp_path / "atlas_v90.json",
    )
    ...
    assert v91_payload["atlasVersion"] == "8.1.91"
    assert v91_payload["validated"] is False
    assert len(v91_payload["lines"]) == 190
    assert sum(len(line["points"]) for line in v91_payload["lines"]) == 18_258

    old_lines = {line["name"]: line for line in v90_payload["lines"]}
    new_lines = {line["name"]: line for line in v91_payload["lines"]}
    ...
    for name in TARGET_NAMES:
        ...
        assert new_lines[name]["postMapTemporalBoundaryMarginFaceRatio"] == 0.01
        ...
        assert np.isclose(new_spec[1][0] - old_spec[1][0], 0.012, atol=1e-12)
        assert np.isclose(new_spec[2][1] - old_spec[2][1], 0.014, atol=1e-12)
    ...
    old_atlas = Atlas.load(str(tmp_path / "atlas_v90.json"))
    new_atlas = Atlas.load(str(tmp_path / "atlas_v91.json"))
    ...
    for frame in frames:
        ...
        for side in ("right", "left"):
            ...
            for name in names:
                ...
                assert -1e-6 <= extension <= 0.0121 * face_width
                ...
                assert new_depth - old_depth > 0.0045 * face_width
                ...
                if available_boundary_space >= 0.01 * face_width:
                    assert boundary_outward - endpoint_outward >= 0.0099 * face_width
                else:
        ...
        assert max(frame_extensions) > 0.009 * face_width
