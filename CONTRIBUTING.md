...
需要 Python 3.10–3.12（mediapipe 暂无 3.13 wheel）。前端需要 Node 24.15+ / npm 11+；在集群上可先 `module load nodejs/24.15.0`。

```bash
# 1. 可编辑安装 + 全部附加项（含检测后端、网页演示、标注工具、测试）
pip install -e ".[all]"
...
可选附加项：`.[mediapipe]` 检测后端 · `.[web]` Gradio 演示 · `.[annotate]` matplotlib 标注 · `.[dev]` 测试/lint。
...
- 遵循 DRY / 单一职责 / 低耦合高内聚 / KISS / YAGNI（见 docs/）。
